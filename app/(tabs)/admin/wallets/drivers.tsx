...
export default function AdminDriverWalletsScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const list = await listDriverWalletSummaries();
  ...
  return (
    <SafeAreaView style={styles.screen} edges={['top']}>
      <AdminHeader title="Driver Wallets" fallbackRoute={adminRoutes.wallets} />
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <Pressable
              style={styles.row}
              onPress={() =>
                router.push(adminRoutes.walletsDriver(item.ownerId) as never)
              }
            >
              <View style={{ flex: 1 }}>
